...
class Message:
	...
	def decode(self, message):
		...
		# convert ccan.message to a bootloader-message
	...
	# --------------------------------------------------------------------------
	def encode(self):
		""" Convert the bootloader-message to a ccan.message """
		
		data = [self.board_id, self.type << 6 | self.subject, self.number, self.data_counter] + self.data
		message = ccan.Message(self.BOOTLOADER_CAN_IDENTIFIER, data, extended = False, rtr = False)
...
class CommandlineClient(Bootloader):
	...
	def _start_bootloader_command(self):
		...
		msg = ccan.Message(int(id, 16), extended = True, rtr = False)
...
if __name__ == '__main__':
	...
	parser.add_option("-t", "--type", dest="type", default="ccan.usb",
			help="Select type of CAN adapter ('ccan.usb' or 'shell' or 'tiny' or 'socket')")
	...
	# create a connection to the ccan.bus
	if options.type == "tiny":
		print ("Interface : tinyCan\n")
		interface = ccan.Tiny(port = options.port, 
								baud = int(options.baudrate, 10),
								debug = debug_mode)
	elif options.type == "ccan.usb":
		print ("Interface : CAN2USB\n")
		interface = ccan.Usb2Can(port = options.port, 
								baud = int(options.baudrate, 10),
								debug = debug_mode)
	elif options.type == "shell":
		print ("Interface : CAN Debugger\n")
		interface = ccan.CanDebugger(port = options.port, 
								baud = int(options.baudrate, 10),
								debug = debug_mode)
	elif options.type == "socket":
		print ("Interface : socketccan")
		interface = ccan.Socket(port = options.port, 
								baud = int(options.baudrate, 10),
								debug = debug_mode)
	else:
